...
for r in csv.DictReader(open(os.path.join(BASE, "PLATILLOS-HSC-final.csv"), encoding="utf-8-sig")):
    ...
    d = dishes.setdefault(p, {"nombre": p, "tiempo": r["tiempo"].strip(), "tipo": r["tipo"].strip(),
        "multMax": num(r["mult_max"]) or 1.5, "img": r["image_filename"].strip(), "fixed": [0, 0, 0, 0], "ings": []})
    ...
    if rol == "principal":
        ...
        if m:
            ...
            ing["a"] = [round(m[i] / 100, 5) for i in range(4)]
        else:
            ing["pend"] = True
    elif rol == "sub-receta":
        t = SUB.get(al)
        if t:
            for i in range(4): d["fixed"][i] += t[i]
    elif rol in ("guarnicion", "fijo"):
        m = CAT.get(al)
        if m:
            for i in range(4): d["fixed"][i] += m[i] * g / 100
    d["ings"].append(ing)
...
ts += "// Cada platillo: fixed = macros [kcal,P,F,C] de todo lo NO-principal (ya sumado);\n"
ts += "// ings principales traen a=[por-gramo] y max para que el motor ajuste la porción.\n"
